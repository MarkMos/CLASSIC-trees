...
from classic_trees import get_tree_vals, get_tree_vals_FoF, functions
...
def tree_process_FoF(i,i_seed_0,mp_halo,a_halo,m_res,m_min,w_lev,a_lev,n_lev,n_halo_max,n_halo,pos_base,vel_base,scaling,Boxsize):
    ...
    arr_MostBoundID = np.zeros(np.sum(count),dtype='int_')
    arr_vel_disp = np.zeros(np.sum(count))
    arr_SubhaloNr = np.zeros(np.sum(count),dtype='int_')
    count = np.array(count,dtype='int_')
    i = np.array([i],dtype='int_')
...
def parallel_exe_FoF(j,n_tree,i_seed_0,mp_halo,a_halo,m_res,m_min,w_lev,a_lev,n_lev,n_halo_max,n_halo,nth_run,start_offset,file_name,omega_0,l_0,h_0,BoxSize,mode,pos_base,vel_base,scaling):
    ...
    with h5py.File(file_name,'a',libver='latest') as f:
        ...
        for result in results:
            ...
            append_create_dataset(grp1,'TreeDescendant',result['arr_desc'])
            append_create_dataset(grp1,'TreeFirstProgenitor',result['arr_1prog'])
            append_create_dataset(grp1,'TreeNextProgenitor',result['arr_nextprog'])
            ...
            append_create_dataset(grp1,'Group_M_Crit200',result['arr_GroupMass'])
            append_create_dataset(grp1,'SubhaloPos',result['arr_pos'])
            append_create_dataset(grp1,'SubhaloVel',result['arr_velo'])
            append_create_dataset(grp1,'SubhaloSpin',result['arr_spin'])
            append_create_dataset(grp1,'SubhaloLen',result['arr_sublen'])
